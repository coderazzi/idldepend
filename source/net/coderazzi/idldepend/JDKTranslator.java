...
/**
 * Translator for the JDK. Following is the behaviour of the parameters
 * -pkgPrefix (translated into the task's parameter package) and -pkgTranslate
 * (translated into translate)<br>
 * 1- pkgPrefix is applied before any pkgTranslate. And pkgTranslate can modify
 * the change done by pkgPrefix<br>
 * 2- pkgPrefix requires always two parameters: <t>, called in the task 'module'
 * and <prefix>, called 'prefix'. 'module' cannot be complex, that is, it cannot
 * include several modules, like in "modA.modB", but prefix can be.<br>
 * 3- The result of applying pkgPrefix is to add the given prefix to any type
 * whose top level module is 'module'. It also adds the prefix to any interface
 * or valuetype defined at top level! This means that an interface called Iface,
 * where -pkgPrefix Iface Add is applied, would produce files like
 * Add/Iface.java, Add/IfaceHelper.java, Add/IfacePackage/..., etc<br>
 ...
 * behaviour applies the same for -pkgTranslate<br>
 * 5- if two -pkgPrefix are specified, with the same 'module', it is the second
 * one that stays. Two different -pkgPrefix are not applied on the same type.<br>
 ...
 * -pkgTranslate Iface Mod is applied.<br>
 * 8- If two pkgTranslate are defined with the same 'module', only the latest
 * specification is used.<br>
 * 9- When several pkgTranslates are specified, they are applied secuentially,
 * in the specification order. But once that a pkgTranslate has modified a type,
 * no more translations are applied. HOWEVER (bug solved on v0.23), if two
 * translates are defined over two modules, one of which include the other, it
 * is applied the more suitable first!<br>
 */
class JDKTranslator extends Translator {
   List packages, translates, inputPackages, inputTranslates;

   public JDKTranslator(List inputPackages, List inputTranslates)
   ...
   /**
    * Verifies that all the packages contain a prefix and a module, and no auto.
    * It also verifies the values for module and prefix, removing any traling
    * spaces if required
    * 
    * @param packages
    *           the input packages
    * @param toUse
    *           a list to include the packages to be used on later translations
    */
      ...
      while (it.hasNext()) {
         ...
         if (task.module == null) {
            throw new BuildException(
                  "Using compiler JDK, every <package> must contain a module attribute");
         }
         if (task.prefix == null) {
            throw new BuildException(
                  "Using compiler JDK, every <package> must contain a prefix attribute");
         }
         if (task.auto) {
            throw new BuildException(
                  "Using compiler JDK, <package> does not accept the attribute auto");
         }
   ...
   /**
    * Verifies that all the translates contain a package and a module It also
    * verifies the values for module and package, removing any trailing spaces
    * if required. If there are two translates with the same moduleName, only
    * the second one is kept. If two translates are not the same but apply to
    * the same module, the larger one is kept first, as it will be more suitable
    * 
    * @param translates
    *           the input translates
    * @param toUse
    *           a list to include the packages to be used on later translations
    */
   private void checkTranslates(List translates, List toUse)
         throws BuildException {
      ...
      while (it.hasNext()) {
         ...
         if (task.moduleName == null || task.packageName == null) {
            throw new BuildException(
                  "Using compiler JDK, every <translate> must contain a module and package attribute");
         }
         String moduleName = normalize(task.moduleName, "module")
               + File.separatorChar;
         ...
         while (true) {
            if (it2.hasNext()) {
               ...
               if (moduleName.startsWith(itsModule)) {
                  if (moduleName.length() == itsModule.length()) { // same
                                                                     // moduleName
                     existingTask.setPackage(packageName);
                  } else { // more suitable, put it before
                     toUse.add(it2.previousIndex(), new TranslateTask(
   ...
   /**
    * Apply the translates defined, until it is found one that modifies the
    * result In JDK, this means to find if the starting of the file matches any
    * of the packages. The matching has to be complete for a module (it does not
    * match part of the name of a module), and this is the reason why the
    * packages were internally stored with a trailing slash
    */
   private String applyTranslates(String file) {
      ...
      while (it.hasNext()) {
         ...
         if (file.startsWith(translate.moduleName)) {
            return translate.packageName
                  + file.substring(translate.moduleName.length());
         }
      }
      return file;
   }
