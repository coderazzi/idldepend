...
/**
 * Translator for OpenORB Following is the behaviour of the parameters -package,
 * -noprefix all of them translated into the task's attribute 'package'<br>
 ...
 * 2- -noprefix requires no parameters. It is specified always, unless a package
 * auto is found!<br>
 * 3- There is no interactions between -noprefix and package<br>
 * 4- If more than one -package is specified, it is used the latest one.<br>
 * 5- It is first applied the pragma prefix (unless -noprefix is specified) and
 * then the package.<br>
 */
class OpenORBTranslator extends Translator {
   ...
   public OpenORBTranslator(List packages, List translates)
         throws BuildException {
      if (!translates.isEmpty()) {
         throw new BuildException(
               "Using compiler OpenORB, <translate> cannot be used");
      }
   ...
   /**
    * Verifies that all the packages contain an auto or/and a prefix. Is several
    * prefixes are defines, is used just the last one
    */
