...
import net.coderazzi.idldepend.idl.IDLMapperUser;
import net.coderazzi.idldepend.idl.VersionsSupporter;
import net.coderazzi.idldepend.javacc.generated.IDLParser;
 ...
 * previously generated, where the dependencies can be obtained, or parsing the
 * specified IDL file (and generating the dependency file for any further
 * checking).
 */
class IDLChecker extends Thread implements IDLMapperUser {
    ...
    * @param targetDir
    *           the directory where the .java files must be located
    * @param includePath
    ...
    *           preprocessor
    * @param client
    *           set to true when the client-side files must be generated
    * @param server
    *           set to true when the server-side files must be generated
    * @param ties
    *           set to true when the server-side delegated-based files must be
    *           generated
    * @param ami
    *           set to true when the ami callbacks must be generated
    * @param checkAll
    *           set to true when the included files are also checked into the
    *           files that must been generated
    * @param translator
    *           the Translator to use to convert the file names
    * @param verbose
    ...
    *           set to true if the IDL syntax must be checked. If not, build()
    *           won't return any valid value, and generatePreprocessedFile is
    *           forced to true
    * @param generatePreprocessedFile
    *           set to true if the preprocessed file must be generated
    * @param generatePreprocessedFileFull
    *           set to true if the preprocessed file must be generated in full
    *           format (expanding as well the include lines). Only used if
    *           generatePreprocessedFile is true
    */
   public IDLChecker(File idlFile, UniqueDependencyId uniqueId,
         File dependsDir, File targetDir, String includePath,
         String addedToPrep, boolean client, boolean server, boolean ties,
         boolean ami, boolean checkAll, Translator translator, boolean verbose,
         boolean debug, Logger logger, boolean checkScopedIncludes, 
         boolean checkDependencies,
         boolean generatePreprocessedFile, boolean generatePreprocessedFileFull)
         throws IOException {
      ...
      try {
         // always create the preprocessor, it will decide wether to preprocess
         // the idl
         // file or not
         preprocessor = new PreprocessingManager(idlFile, uniqueId, dependsDir,
               includePath, addedToPrep, verbose, debug, logger,
               checkScopedIncludes, generatePreprocessedFile, 
               generatePreprocessedFileFull, checkAll);

         // however, the checker is only created if the dependencies must be
         // checked
    ...
    */
   public boolean build(VersionsSupporter versionsSupporter,
         boolean throwExceptionOnError) throws Exception {
      PipedWriter output = null;
      PrintWriter writer = null;
      DependenciesChecker useChecker = checker;

      // if there us not need to check the dependencies, build with return
      // 'false'
      if (useChecker == null) {
         resultReady = true;
         buildResult = false;
      ...
      // are up-to-date. If the dependency file exists, there is no need to
      // parse
      // the IDL file, it is enough to verify the outcome of those dependencies.
         ...
         // in the final case, the IDL file is read, and the dependencies
         // generated
         output = new PipedWriter();
         writer = new PrintWriter(output);
         try {
            idlParser = new IDLParser(new PipedReader(output),
                  versionsSupporter.createIdlToJavaMapping(this, client,
                        server, ties, ami));
      ...
      // independently of the dependencies checking, it must be checked the
      // preprocessor
      // next call will start the preprocessor, which feeds the dependencies
      // checker, and
      // is as well able to generate the preprocessed file
      Exception preprocessingError = preprocessor.process(writer, useChecker,
            useChecker != null);
      ...
      if ((throwExceptionOnError || preprocessingError != null)
            && (buildException != null)) {
   ...
   /**
    * Thread public method. It starts the IDLParser on a second thread.
    */
   public void run() {
   ...
   /**
    * IDLMapperUser public method, issued by the IDL parser when a new type is
    * found to be required.
    */
   public void fileNeeded(String pragmaPrefix, String file, String iface,
         boolean typeInInterface) {
      if (!resultReady) {
         String translatedFile = translator.translate(pragmaPrefix, file,
               iface, typeInInterface);
