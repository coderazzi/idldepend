...
/**
 * Translator for the Jacorb compiler Following is the behaviour of the
 * parameters -p (translated into the task's parameter package) and -i2jpackage
 * (translated into translate) <br>
 * 1- -p has just one attribute, the prefix to be added to every generated file.
 * This prefix can be compound (like in modulA.moduleB). If it is specified
 * several times, only the last one is taken. <br>
 * 2- -i2jpackage requires two parameters: the module to modify and the package
 * on which it is translated. The module cannot be compound, but the replacing
 * package does can. <br>
 * 3- i2jpackage does not modify interfaces or valuetypes themselves. That is,
 * an interface called Iface produces the same files although a translation like
 * -i2jpackage Iface:change is provided. Nevertheless, it does applies to types
 * defined inside an interface. If the previous Iface interface defines a type,
 * this type will generate on standard way a file called IfacePackage/type.idl
 * Applying -i2jpackage Iface:change, the generated file would be
 * change/type.idl (note that it has been specified Iface and not IfacePackage)
 * <br>
 ...
 * where, as soon as a translation is performed, the rest is dismissed. <br>
 * 6- The order of applicance is: first every i2jpackage specified, and then the
 * package (if given).
 * 
 * The version 1.x supports the specification of a global prefix through the
 * argument '-p'. In version 2.x, this argument dissapears, but the same goal
 * can be accomplished by specifying -i2jpackage :prefix The version 1.x accepts
 * the "-i2jpackage :prefix" input, but it does not has the same effect as on
 * version 2.x. For this reason, this translator requires knowing which Jacorb
 * version is handling
 */
class JacorbTranslator extends Translator {
   ...
   public JacorbTranslator(List packages, List translates, boolean version2)
   ...
   /**
    * Verifies that all the packages contain a module, and no prefix or auto. It
    * also verifies the values for prefix, removing any traling spaces if
    * required
    */
      ...
      while (it.hasNext()) {
         ...
         if (task.module != null || task.auto) {
            throw new BuildException(
                  "Using compiler Jacorb, <package> does not accept the attribute auto or module");
         }
         if (task.prefix == null) {
            throw new BuildException(
                  "Using compiler Jacorb, every <package> must contain a prefix attribute");
         }
    ...
    * verifies the values for module and package, removing any traling spaces if
    * required. It stores the modules in the HashMap
    */
   private void checkTranslates(List translates) throws BuildException {
      ...
      while (it.hasNext()) {
         ...
         if (task.moduleName == null || task.packageName == null) {
            throw new BuildException(
                  "Using compiler Orbacus, every <translate> must contain a module and package attribute");
         }
   ...
   private String applyPackages(String file) {
      return packageString == null ? file : packageString + File.separatorChar
            + file;
   }
