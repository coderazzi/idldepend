...
public interface PreprocessorInterface {
   ...
   /**
    * Called by the preprocessor if an asynchronous exception parsing the file
    * is found
    */
   ...
   /**
    * Called by the preprocessor when an #include tag has been found. The
    * specified file includes everything specified after the #include tag,
    * including any '<' or '>' characters
    * 
    * @param file
    *           The string containing the file to include. It includes any
    *           character found after the #include chars (without the spaces on
    *           both sides)
    * @param includer
    *           The FileIncluder requesting this file
    */
   public File includeFile(String file, FileIncluder includer)
         throws ParseException;

   /**
    * Called by the preprocessor when there is a file that could be included,
    * depending on the current state (is included unless there is a conditional
    * block resolved to be NOT executed).
    */
    ...
    * In this call, they're only specified the macro name and the list with the
    * parameter names, which can be empty. This call is followed by several
    * calls to readToken and then one to macroDefined
    */
   public void defineMacro(String macroName, List macroParameters)
         throws ParseException;

   /**
    * Called by the preprocessor when a #define macro without arguments is
    * found. In this call, it's only specified the macro name.This call is
    * followed by several calls to readToken and then one to macroDefined
    */
    ...
    * which must be stored for some purpose (for example, it is called on an
    * include directive to store the file name, or in the macro definition).
    ...
    * 
    * @param sameLevel
    *           is set to true for #elif directives, and false for #if
    */
    ...
    * 
    * @param sameLevel
    *           is set to true for #endif directives, and false for #else
    */
   public void alternateEvaluationLevel(boolean closeLevel)
         throws ParseException;
    ...
    * 
    * @param id
    *           the name of the macro to be checked
    * @param defined
    *           is set to true fo #ifdef, and not for #ifndef
    */
