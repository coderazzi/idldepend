...
/**
 * Main preprocessor task. It accomplishes the following tasks:<br>
 * -Joins lines ending with '\'<br>
 * -Removes out comments, being replaced by spaces.<br>
 * -Separates preprocessor lines (beginning with #) from the others<br>
 *
 * The output of this class is redirected to a Reader object, making
 * therefore possible for a class to 'read' from its output:<br>
 * -Through this output, is redirected the file's content, already filtered.<br>
 * -The preprocessor lines are converted into //line<br>
 *   For example: /*comment* /  #  define /*comment* / macro<br>
 *   is converted into //define macro<br>
 *
 * Why this is not done directly with javacc?<br>
 ...
 *    Probably this can be easily done, I have not that expertise.<br>
 *
 ...
 *    For example, I can define #define A(include) #include<br>
 *
 ...
 *    even skipped on the current file.<br>
 *    NOTE THAT THIS THIRD CASE IS PERHAPS WRONG. THE GNU PREPROCESSOR DOES
 *    NOT SUPPORT THIS BEHAVIOUR<br>
 *
 ...
 * a directive line: this assumption is not taken here.<br>
 *
 ...
 *  syntax ////line number file<br>
 * Note that it uses four initial slashes to differenciate it from other
 *  #line directives that can be existing on the initial file<br>
 */
public class Prepreprocessor extends Thread implements FileIncluder {
    ...
    */
   public Prepreprocessor(File file, PreprocessorInterface controller,
         String addInfo) throws ParseException, IOException {
         ...
         if (length > 0 && line.charAt(length - 1) == '\\') {
            ...
            if (parseLine()) {
               try {
                  ...
                  File toInclude = controller.includeFile(read.substring(9)
                        .trim(), this);
                  if (toInclude == null) {
                     writer.println();
                  } else {
                  ...
                  throw new LocatedParseException(pex, (addInfo == null ? file
                        .toString() : null), input.getLineNumber());
               }
            } else {
    ...
    */
   private void passCharacter(char pass) throws ParseException {
      while (++pos < len) {
         char c = read.charAt(pos);
         if (c == '\\') {
            ++pos; // do not read next character
      ...
      throw new LocatedParseException(
            "Line does no complete is character/string" + " literal",
