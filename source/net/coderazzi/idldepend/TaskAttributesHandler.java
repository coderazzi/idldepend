...
public class TaskAttributesHandler {
   ...
   private boolean jacorb2; //if it is true, jacorb is true as well
    ...
    */
   public TaskAttributesHandler(CompilerAttribute compiler,
         Commandline commandline, String idlCompiler) {
      if (idlCompiler != null) {
      ...
      if (compilerValue.equals(JDK)) {
         ...
         if (!forceExternalExecutable) {
            commandline
                  .setExecutable("com.sun.tools.corba.se.idl.toJavaPortable.Compile");
         }
    ...
    */
   public void execute(Commandline commandline, List idlFiles, Path classpath,
         Path path, Task owner) {
      if (verboseBASIC) {
      ...
      } else if (jacorb | openorb) //executed in a forked process
      {
         ...
         if (path != null) {
            owner
                  .log("Warning: path specified, but cannot be applied with the chosen compiler");
         }
         ...
         if (jacorb2){
             executer.createArg().setValue("-forceOverwrite");
         }
         Iterator it = idlFiles.iterator();
         while (it.hasNext()) {
         ...
         if (verboseDEBUG) {
            owner.log("Execute: " + executer.getCommandLine().getJavaCommand());
         }
         executer.execute();
   ...
   private String getFileNames(List files) {
      ...
      while (it.hasNext()) {
         sb.append(" \"").append((File) it.next()).append("\"");
      }
      return sb.toString();
   }
    ...
    */
   private void runCommand(Task task, String[] cmdline, Path classpath,
         Path path, Task owner) throws BuildException {
      try {
         task.log(Commandline.describeCommand(cmdline), Project.MSG_VERBOSE);
         Execute exe = new Execute(new LogStreamHandler(task, Project.MSG_INFO,
               Project.MSG_ERR));
         ...
         throw new BuildException(
               "Could not launch " + cmdline[0] + ": " + exc, task
                     .getLocation());
      }
   }
   ...
   public Translator createTranslator(List packages, List translates) {
      ...
      if (jdk) {
         ...
         ret = new JacorbTranslator(packages, translates, jacorb2);
      } else {
   ...
   public void handleSideAttribute(SideAttribute side, Commandline commandLine) {
      ...
      generateClient = !sideValue.equals(SERVER)
            && !sideValue.equals(SERVERTIE);
      ...
      if (jdk) {
         ...
         if (!generateClient) {
            throw new BuildException("Compiler " + ORBACUS
                  + " does not accept " + SERVER + " or " + SERVERTIE
         ...
         if (generateServer && !generateTIE) {
            throw new BuildException("Compiler " + JACORB + " does not accept "
                  + SERVER + " or " + ALL + " on the attribute <side>");
         }
      } else { // orbix2k
         if (generateServer) {
         ...
         if (generateTIE) {
            throw new BuildException("Compiler " + ORBIX2K
                  + " does not accept " + SERVERTIE
                  + " on the attribute <side>");
         }
      }
   }
   ...
   private String getCheckAllFlag() {
      if (orbix2k) {
         throw new BuildException("Compiler " + ORBIX2K
               + " does not accept <checkALL> elements");
      }
      return jdk ? "-emitAll" : orbacus ? "--all" : "-all";
   }
   ...
   private String getAMIFlag(String AMI) //just now, only CALLBACK, no checks
   {
      ...
      if (jacorb) {
         ...
         throw new BuildException(
               "This compiler does not accept <ami> elements");
      }
      return ret;
   }
   ...
   private String getVerboseFlag(VerboseLevel level) {
      ...
      if (level.getValue().equals(DEBUG)) {
         ret = (jdk || orbix2k) ? "-v" : orbacus ? "-d" : jacorb ? "-W 4"
               : openorb ? "-verbose" : null;
   ...
   public static class SideAttribute extends EnumeratedAttribute {
      ...
      public boolean isServerTie() {
         return getValue().equals(SERVERTIE);
      }
   ...
   public static class CompilerAttribute extends EnumeratedAttribute {
      ...
      public boolean isOpenORB() {
         return getValue().equals(OPENORB);
      }

      public boolean isOrbacus() {
         return getValue().equals(ORBACUS);
      }

      public boolean isJdkOrIbm() {
         return getValue().equals(JDK) || getValue().equals(IBMJDK);
      }

      public boolean isJacorb() {
          return getValue().equals(JACORB) || getValue().equals(JACORB2);
       }
