...
import net.coderazzi.idldepend.javacc.generated.Preprocessor;
 ...
 */
public class PreprocessorController implements PreprocessorConstants,
   ...
   public void defineMacro(String macroName, List macroParameters) {
      if (validLevel) {
         macro = new Macro(macroName, macroParameters == null ? new ArrayList()
               : macroParameters);
   ...
   public void undefineMacro(String macroName) {
      if (validLevel) {
         ...
         if (previous != null && previous.isInternal()) {
            user.warning("Internal macro cannot be undefined: "
                  + macro.getName());
            macroHandler.addMacro(previous);
         }
      }
   ...
   public void readToken(Token token) throws ParseException {
      switch (tokenStatus) {
      case DISCARD_TOKEN:
         if (token.kind == PreprocessorConstants.EOL) {
            //even if we are discarding tokens, we want to preserve the
            //line numbers
   ...
   public void completeEvaluation(boolean sameLevel) throws ParseException {
      ...
      validLevel = sameLevel ? ifLevel.addElif(evaluation) : ifLevel
            .addIf(evaluation);
      tokenStatus = validLevel ? TRANSFORM_TOKEN : DISCARD_TOKEN;
   }
