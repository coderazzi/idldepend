 ...
 * files, being important the order on which they are specified.<br>
 * The rules to find included files are the following:<br>
 ...
 *    The preprocessor then searches along the path specified by the user<br>
...
class IncludesHandler {
    ...
    */
   public File findFile(String path, FileIncluder fileIncluder)
      ...
      if (inMap == null) {
         ...
         if (wrong) {
            throw new ParseException("Include file incorrectly specified: "
                  + path);
         }
